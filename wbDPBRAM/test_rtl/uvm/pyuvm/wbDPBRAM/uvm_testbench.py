...
class wbDPBRAMDriver(uvm_driver):
...
class wbDPBRAMMonitor(uvm_monitor):
...
class wbDPBRAMAgent(uvm_agent):
    def build_phase(self):
        self.monitor = wbDPBRAMMonitor.create("monitor", self)
        self.driver = wbDPBRAMDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class wbDPBRAMScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class wbDPBRAMEnv(uvm_env):
    def build_phase(self):
        self.agent = wbDPBRAMAgent.create("agent", self)
        self.scoreboard = wbDPBRAMScoreboard.create("scoreboard", self)
...
class wbDPBRAMTest(uvm_test):
    def build_phase(self):
        self.env = wbDPBRAMEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("wbDPBRAMTest")