...
class wbTDPBRAMDriver(uvm_driver):
...
class wbTDPBRAMMonitor(uvm_monitor):
...
class wbTDPBRAMAgent(uvm_agent):
    def build_phase(self):
        self.monitor = wbTDPBRAMMonitor.create("monitor", self)
        self.driver = wbTDPBRAMDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class wbTDPBRAMScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class wbTDPBRAMEnv(uvm_env):
    def build_phase(self):
        self.agent = wbTDPBRAMAgent.create("agent", self)
        self.scoreboard = wbTDPBRAMScoreboard.create("scoreboard", self)
...
class wbTDPBRAMTest(uvm_test):
    def build_phase(self):
        self.env = wbTDPBRAMEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("wbTDPBRAMTest")